...
export function useDrivePage(options: { page: Ref<number>, size: Ref<number> }) {
  ...
  const order = shallowRef<Sorter['o']>()
  const asc = shallowRef<Sorter['asc']>()
  const fc_mix = shallowRef<Sorter['fc_mix']>()
  ...
  function changeSort(o: Sorter['o'], a: Sorter['asc'], f: Sorter['fc_mix']) {
